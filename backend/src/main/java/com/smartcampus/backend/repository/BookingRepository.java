...
public interface BookingRepository extends MongoRepository<Booking, String> {
    ...
    List<Booking> findByFacilityIdAndStatusOrderByBookingStartAsc(
            String facilityId,
            Booking.BookingStatus status
    );

    // Full facility time conflict
    @Query("{ 'facilityId': ?0, 'status': 'APPROVED', '$or': [" +
           "{ 'bookingStart': { '$lt': ?2 }, 'bookingEnd': { '$gt': ?1 } }" +
           "] }")
    List<Booking> findApprovedConflictingBookings(
            String facilityId,
            LocalDateTime start,
            LocalDateTime end
    );
    ...
    @Query("{ 'facilityId': ?0, 'status': 'APPROVED', '_id': { '$ne': ?3 }, '$or': [" +
           "{ 'bookingStart': { '$lt': ?2 }, 'bookingEnd': { '$gt': ?1 } }" +
           "] }")
    List<Booking> findApprovedConflictingBookingsExcluding(
            String facilityId,
            LocalDateTime start,
            LocalDateTime end,
    ...
    // Seat conflict with time
    @Query("{ 'facilityId': ?0, 'status': { '$in': ['PENDING', 'APPROVED'] }, " +
           "'bookingStart': { '$lt': ?2 }, 'bookingEnd': { '$gt': ?1 }, " +
           "'seatNumbers': { '$in': ?3 } }")
    List<Booking> findSeatConflicts(
            String facilityId,
            LocalDateTime start,
            LocalDateTime end,
